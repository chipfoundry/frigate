...
# Input delays
set_input_delay -max 12 -clock [get_clocks {clk}] [all_inputs]
set_input_delay -min 10 -clock [get_clocks {clk}] [all_inputs]
set_input_delay -max 16 -clock [get_clocks {clk}]  [get_ports {HADDR[*]}]
set_input_delay -min 3 -clock [get_clocks {clk}] [get_ports {HADDR[*]}]

# Input Transition
set_input_transition -max 0.20  [all_inputs]

# Output delays
set_output_delay -max 12  -clock [get_clocks {clk}] [all_outputs]
set_output_delay -min 10 -clock [get_clocks {clk}] [all_outputs]
...
set_multicycle_path -setup 2 -from [get_ports {HADDR*}] -through [get_ports {HRDATA*}]
set_multicycle_path -hold  1 -from [get_ports {HADDR*}] -through [get_ports {HRDATA*}]
